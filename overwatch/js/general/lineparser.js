...
function init(meta) {
    ...
    function _help() {
        ...
        if (meta.options) {
            const options = [];
            if (meta.options.flags) {
                for (const flag of meta.options.flags) {
                    options.push([
                        (flag[0] !== null ? `-${flag[0]},` : ""),
                        (flag[1] !== null ? `--${flag[1]} ` : ""),
                        flag[2],
            ...
            if (meta.options.parameters) {
                for (const param of meta.options.parameters) {
                    options.push([
                        (param[0] !== null ? `-${param[0]},` : ""),
                        (param[1] !== null ? `--${param[1]} ` : ""),
                        param[2] + (param[3] !== null ? ` (= ${param[3]})` : ""),
                    ]);
                }
            }
            if (options.length > 0) {
                ...
                for (const option of options) {
                    for (let i = 0; i < 3; ++i) {
                        maxLength[i] = Math.max(maxLength[i], option[i].length);
                    }
                }
                const spaces = [""];
                const maxMaxLength = maxLength.reduce((a, b) => Math.max(a, b), 0);
                for (let i = 1; i <= maxMaxLength; ++i) {
                    spaces[i] = spaces[i-1]+" ";
                }
                s += options.map(option =>
                    "  " + option.map((text, index) => text+spaces[maxLength[index]-text.length])
                        .filter(text => text !== "")
                        .join(" ")
                ).join("\n");
            }
